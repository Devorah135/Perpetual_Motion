...
public class CardPilesAdapter extends RecyclerView.Adapter<CardPileTopViewHolder> {
    ...
    private void setTextOfEachTextView(CardPileTopViewHolder holder, Card currentCard) {
        String rankValue = String.format(Locale.getDefault(), "%d", currentCard.getRank());
    ...
    private int getCurrentColorWithNightMode(Card currentCard) {
        ...
        if (currentColor == Color.BLACK)
            currentColor = mIS_NIGHT_MODE ? Color.LTGRAY : currentColor; // if color is black, if it's night, make color lightgray, otherwise if not night, keep current color (black)
        return currentColor;
    }
    ...
    public int getItemCount() {
        return mPILE_TOPS.length; // Going to be 4
    }
    ...
    public void overwriteChecksForm (boolean[] newChecksSet) {
        System.arraycopy (newChecksSet,
                0,
                mCHECKED_PILES,
                0,
